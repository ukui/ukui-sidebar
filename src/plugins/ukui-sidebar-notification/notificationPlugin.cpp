...
NotificationPlugin::NotificationPlugin()
{
    ...
    this->setLayout(pNotificationVBoxLayout);
    ...
    //悬浮收纳数标签
    m_pTakeInCoutLabel = new QLabel(this);
    ...
    m_pQScrollAreaNotify->adjustSize();
    ...
    if (QGSettings::isSchemaInstalled(UKUI_TRANSPARENCY_SETTING)) {
        QGSettings* pTransparency = new QGSettings(UKUI_TRANSPARENCY_SETTING, "", this);
        m_dTranSparency = pTransparency->get("transparency").toDouble();
        connect(pTransparency, &QGSettings::changed, this, [=](QString value) {
            m_dTranSparency = pTransparency->get("transparency").toDouble();;
        });
    }
...
void NotificationPlugin::paintEvent(QPaintEvent *e)
{
    QPainter p(this);
